...
lapply(response_cols, function(response) {
  print(paste('Analysis for', response))
  print(model_response(master, response, TRUE))
  print('-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-')
})
```
...
```{r}
response <- 'average_a'
# response_formula <- as.formula(paste(response, "~ trt*muscle*display + (1 | batch)  + (1 | index) + (1 | batch:index)"))
response_formula <- as.formula(paste(response, "~ trt*muscle*display + (1 | batch)"))
mod <- lmer(response_formula, data = master)
emmeans(mod, specs = ~ trt:muscle:display, infer = c(TRUE, TRUE))
summary(mod)
```

```{r}
library(ggeffects)
ggpredict(mod, terms = c('trt', 'muscle', 'display', 'batch')) %>% plot()
```
